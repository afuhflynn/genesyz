...
export function CohortManagement({
  ...
  const fetchCohortStartups = async (cohortId: string) => {
    try {
      ...
      const res = await fetch(
        `/api/accelerators/${slug}/cohorts/${cohortId}/startups`,
      );
      const data = await res.json();
      if (data.data) {
  ...
  const onboardStartup = async (startupId: string) => {
    ...
    try {
      const res = await fetch(
        `/api/accelerators/${slug}/cohorts/${selectedCohort.id}/startups`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          ...
          {canManageCohorts && (
            <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
              ...
              <DialogContent>
                <Form {...form}>
                  <form
                    onSubmit={form.handleSubmit(onCreateCohort)}
                    className="space-y-4"
                  >
                    <DialogHeader>
                      <DialogTitle>Create New Cohort</DialogTitle>
                      <DialogDescription>
                        Add a new batch of startups to your program.
                      </DialogDescription>
                    </DialogHeader>
        ...
        {selectedCohort ? (
          <>
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  {selectedCohort.description || "Active batch management."}
                </p>
              </div>
                  ...
                  <Dialog>
                    ...
                    <DialogContent className="sm:max-w-[500px]">
                      <DialogHeader>
                        ...
                        <DialogDescription>
                          Search for startups on Genesyz to add them to{" "}
                          {selectedCohort.name}.
                        </DialogDescription>
                      </DialogHeader>
                      <div className="space-y-4 py-4">
                        <div className="flex gap-2">
                          <Input
                            ...
                            onKeyDown={(e) =>
                              e.key === "Enter" && onSearchStartups()
                            }
                          />
                          <Button
                            size="icon"
                            onClick={onSearchStartups}
                            ...
                            >
                              <div>
                                ...
                                <p className="text-[10px] text-muted-foreground">
                                  {s.user.email} • {s.industry || "General"}
                                </p>
                              </div>
                              <Button
                                size="sm"
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Track the latest updates and health of all startups in this
                  batch.
                </CardDescription>
              </CardHeader>
              <CardContent>
                  ...
                  <TableBody>
                    {isLoadingStartups ? (
                        ...
                        <TableRow key={s.startup.id}>
                          ...
                          <TableCell className="text-sm text-muted-foreground">
                            {s.startup.weeklyUpdates[0]
                              ? new Date(
                                  s.startup.weeklyUpdates[0].createdAt,
                                ).toLocaleDateString()
                              : "No updates yet"}
                          </TableCell>
                          <TableCell>
                            {s.startup.weeklyUpdates[0] ? (
                              <div className="flex items-center gap-1">
                                <div
                                  className={`h-2 w-2 rounded-full ${s.startup.weeklyUpdates[0].moraleScore > 7 ? "bg-emerald-500" : s.startup.weeklyUpdates[0].moraleScore > 4 ? "bg-amber-500" : "bg-destructive"}`}
                                />
                                <span className="text-sm">
                                  {s.startup.weeklyUpdates[0].moraleScore}/10
                                </span>
                              </div>
                            ) : (
                              "-"
                      ...
                      <TableRow>
                        ...
                        >
                          No startups in this cohort yet. Click "Onboard
                          Startups" to add some.
                        </TableCell>
                      </TableRow>
                    )}
                  </TableBody>
          ...
          <div className="flex h-[400px] flex-col items-center justify-center border-2 border-dashed rounded-xl text-center p-8">
            ...
            <p className="text-sm text-muted-foreground max-w-xs mb-6">
              Select a cohort from the sidebar or create a new one to start
