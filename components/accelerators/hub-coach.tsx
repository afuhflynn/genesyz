...
export function HubCoach({ slug }: { slug: string }) {
  ...
  const runAnalysis = async () => {
    try {
      setIsAnalyzing(true);
      const res = await fetch(`/api/accelerators/${slug}/coach`, { method: "POST" });
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Strategic oversight and cohort-wide pattern recognition.</p>
        </div>
      ...
      {!analysis && !isAnalyzing ? (
        <Card className="border-dashed flex flex-col items-center justify-center p-12 text-center">
           ...
           <p className="text-sm text-muted-foreground max-w-sm mb-6">
              The Hub Coach will scan all startups in your cohorts, their weekly updates, and your program KPIs to provide a strategic health report.
           </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <Card className="md:col-span-2 border-primary/20 bg-primary/5">
             <CardHeader>
                <div className="flex items-center justify-between">
                   <CardTitle className="text-lg">Cohort Health Synthesis</CardTitle>
                   <Badge variant={analysis.overview.sentiment === "EXCELLENT" ? "default" : "destructive"}>
                      {analysis.overview.sentiment}
                   </Badge>
                </div>
                <CardDescription>Strategic summary based on latest updates.</CardDescription>
             </CardHeader>
             <CardContent className="space-y-6">
                <p className="text-sm leading-relaxed">{analysis.overview.summary}</p>
                <div className="bg-background/80 p-4 rounded-lg border">
                   <p className="text-[10px] font-bold uppercase text-primary mb-1">Critical Bottleneck</p>
                   <p className="text-sm font-medium">{analysis.overview.topBottleneck}</p>
                </div>
                ...
                <div className="space-y-4">
                   ...
                   <div className="grid gap-3">
                      {analysis.cohortPatterns.map((p, i) => (
                         <div key={i} className="text-xs p-3 border rounded-md bg-background">
             ...
             <Card className="border-destructive/20 shadow-none">
                <CardHeader className="py-4">
                   <CardTitle className="text-sm flex items-center gap-2">
                      <AlertCircle className="h-4 w-4 text-destructive" /> At-Risk Startups
                   </CardTitle>
                </CardHeader>
                <CardContent className="py-4 space-y-4">
                   {analysis.atRiskStartups.map((s, i) => (
                      <div key={i} className="space-y-1">
                         ...
                         <p className="text-[10px] text-destructive italic">{s.suggestedIntervention}</p>
                         {i < analysis.atRiskStartups.length - 1 && <Separator className="mt-3" />}
                      </div>
                   ))}
                </CardContent>
             </Card>
             ...
             <Card className="border-emerald-500/20 shadow-none bg-emerald-50/10">
                ...
                <CardContent className="py-4 space-y-2">
                   <div className="flex items-center gap-2">
                      <div className={`h-2 w-2 rounded-full ${analysis.kpiForecast.onTrack ? 'bg-emerald-500' : 'bg-amber-500'}`} />
                      <span className="text-xs font-medium">{analysis.kpiForecast.onTrack ? "On Track" : "Lagging Targets"}</span>
                   </div>
