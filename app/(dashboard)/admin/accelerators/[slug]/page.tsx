...
import { BarChart3, Users, TrendingUp, AlertTriangle, Calendar, Settings, Shield } from "lucide-react";
...
async function getAcceleratorData(slug: string) {
  ...
  const totalStartups = accelerator.cohorts.reduce((acc, c) => acc + c._count.startups, 0);
  const activeFlags = accelerator.cohorts.reduce((acc, c) => {
    return acc + c.startups.reduce((sAcc, s) => sAcc + s.startup.flags.length, 0);
  }, 0);
...
export default async function AcceleratorDashboardPage({
  ...
  const { hasAccess, role } = await checkAcceleratorAccess(slug, "view_metrics");
  ...
  return (
    <div className="flex flex-col gap-8 pb-12">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">{accelerator.name}</h1>
          <p className="text-muted-foreground">
            {accelerator.programType.charAt(0).toUpperCase() + accelerator.programType.slice(1)} Dashboard • {role?.replace("_", " ")}
          </p>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">Across {accelerator.cohorts.length} active cohorts</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-destructive">{activeFlags}</div>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">Cohort average WoW growth</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{accelerator._count.applications}</div>
            <p className="text-xs text-muted-foreground">Next intake applications</p>
          </CardContent>
        </Card>
      </div>
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-7">
            <Card className="col-span-4">
              <CardHeader>
                ...
                <CardDescription>Aggregate metrics across all active startups.</CardDescription>
              </CardHeader>
            ...
            <Card className="col-span-3">
              <CardHeader>
                ...
                <CardDescription>Hub-level goals set by managers.</CardDescription>
              </CardHeader>
              <CardContent>
          ...
          <div className="grid gap-4 md:grid-cols-1 lg:grid-cols-7">
             <Card className="col-span-7">
                <CardHeader>
                   ...
                   <CardDescription>Startups requiring intervention due to missed updates or poor metrics.</CardDescription>
                </CardHeader>
                <CardContent>
                   <div className="space-y-4">
                      {accelerator.cohorts.flatMap(c => c.startups).filter(s => s.startup.flags.length > 0).length > 0 ? (
                         accelerator.cohorts.flatMap(c => c.startups).filter(s => s.startup.flags.length > 0).map((s) => (
                            <div key={s.startup.id} className="flex items-center justify-between border-b pb-4 last:border-0">
                               <div className="space-y-1">
                                  <p className="text-sm font-medium leading-none">{s.startup.name}</p>
                                  <p className="text-xs text-muted-foreground">
                                     {s.startup.flags[0].reason}
                                  </p>
                               </div>
                               <div className="flex items-center gap-2">
                                  <Badge variant={s.startup.flags[0].severity === "critical" ? "destructive" : "warning"}>
        ...
        <TabsContent value="settings" className="space-y-6">
           ...
           <Card>
              <CardHeader>
                 ...
                 <CardDescription>Update your program branding and public visibility.</CardDescription>
              </CardHeader>
              <CardContent>
